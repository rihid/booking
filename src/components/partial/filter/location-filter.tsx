...
import { ChevronDownIcon, MapPin, XIcon } from 'lucide-react';
...
function ComboboxLocation({
  ...
  const [locationParam, setLocationParam] = useQueryState(
    'location',
  ...
  // Local state for input query
  const [query, setQuery] = React.useState<string>('');
  
  // Find current location from URL parameter
  const currentLocation = React.useMemo(() => {
    return locations.find(loc => loc.name === locationParam) || null;
  }, [locationParam, locations]);

  // Filter locations based on search query
  const filteredLocation = query === '' 
    ? locations 
  ...
  const handleLocationSelect = (value: LocationType | null) => {
    ...
    setLocationParam(value ? value.name : null);
  };
  ...
  // Sync input with URL parameter on first load and changes
  React.useEffect(() => {
      ...
      <Combobox value={currentLocation} onChange={handleLocationSelect}>
        <div className="relative">
          <ComboboxInput
            ...
            className={cn(
              'w-full rounded-full border-none bg-background px-3 py-2 pl-3 text-sm text-muted-foreground',
