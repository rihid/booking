...
import CaptainRatingForm from './captain-rating-form';
import ProductList from './product-list';
import DownloadInvoiceBtn from './download-invoice-btn';
...
async function InvoicePageContent({
  ...
  const { token, customer_no, org_no } = user;
  ...
  const invoice = invoices.find(inv => inv.invoice_no.split('/').pop() === noShort);
  ...
  return (
    <>
      ...
      <PriceDetailComp />
      <DownloadInvoiceBtn user={user} invoice={invoice} />
    </>
  )
}
